...
st.set_page_config(page_title="RescueAI Shelters", layout="wide")
...
with shelter_tab:
    ...
    try:
        res = requests.get(f"http://localhost:8000/api/shelters/nearby?lat={lat_in}&lng={lng_in}&limit=6", timeout=5)
        if res.status_code == 200:
                    ...
                    st.markdown(
                        ...
                        <div class="glass-card">
                            ...
                            <div style="margin: 12px 0;">
                                <span style="font-size: 0.85rem; color: #9fb0c3;">Occupancy Capacity ({s['total_beds'] - s['available_beds']}/{s['total_beds']} beds occupied):</span>
                            </div>
                        </div>
                        """,
                        unsafe_allow_html=True
                    )
                    # Render Streamlit progress bar for occupancy
                    st.progress(occ_pct)
                    
                    # Amenities Icons / Indicators
            ...
            st.error("Failed to load shelters from backend API.")
...
with hospital_tab:
    ...
    try:
        res = requests.get(f"http://localhost:8000/api/hospitals/nearby?lat={lat_in}&lng={lng_in}&limit=6", timeout=5)
        if res.status_code == 200:
            hospitals = res.json()
            if not hospitals:
            ...
            st.error("Failed to load hospitals from backend API.")
    except Exception as e:
        st.error(f"Cannot connect to API backend ({e})")
